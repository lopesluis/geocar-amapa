...
from qgis.core import (
    ...
    QgsWkbTypes, QgsField, QgsFields, QgsApplication,
)
...
BASE_SHA256 = "78f470878e855a62a5574a92f151b30fcaa0b37867c88c1f580c40d186f07ec6"
    ...
    def _baixar_base_referencia(self):
        ...
        try:
            requisicao = urllib.request.Request(
                BASE_URL,
                headers={"User-Agent": "GeoCAR-Amapa-QGIS/1.2"}
            )
            with urllib.request.urlopen(requisicao, timeout=60) as resposta:
                ...
                with open(arquivo_temporario, "wb") as destino:
                    while True:
                        ...
                        if total > 0:
                            percentual = min(99, int(baixado * 100 / total))
                            progresso.setValue(percentual)
                            progresso.setLabelText(
                                "Baixando Base de Referência...\n"
                                f"{baixado / (1024 * 1024):.1f} MB de "
                                f"{total / (1024 * 1024):.1f} MB"
                            )
                        else:
                            progresso.setLabelText(
                                "Baixando Base de Referência...\n"
                                f"{baixado / (1024 * 1024):.1f} MB recebidos"
                            )
            ...
            if os.path.exists(arquivo_temporario):
                os.remove(arquivo_temporario)
            self._log(f"Falha no download da Base de Referência: {erro}", "erro")
            QMessageBox.critical(
                self,
                "Falha no download",
                "Não foi possível baixar a Base de Referência. Verifique sua "
